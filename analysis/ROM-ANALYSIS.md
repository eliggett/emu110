...
because its A4 is a **doublet**, at 439.71 and 443.24 Hz -- 3.53 Hz apart, 13.8 cents,
beating at 3.24 Hz (re-measured on `listen/hardware/1`, note 69).

`[C]` **That doublet is the tone's own detune, not chorus** -- see correction #32. P-46
stores output mode index 21, i.e. mode 22 `M31`, whose effect config byte is `0x00`: chorus
and tremolo are both disabled, and the recording is mono to a correlation of 1.00000. Two
partials of nearly equal level at a fixed, stable interval is a Detune-type tone, two voices
a set distance apart. A delay-based chorus does not produce that.
