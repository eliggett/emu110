...
def text_shape_ids(flat_path):
    """The ids of every path in a flattened document that used to be text.

    Needed because lettering wants a finer antialiasing fringe than curves do, and
    after flattening there is nothing structural left to tell them apart.  What does
    survive is the style: Inkscape leaves font-family on the paths it makes from a
    <text>, and puts it on nothing else.

    Matching on that rather than on the ids the exporter already knows is what makes
    this work for a MULTI-LINE label.  Inkscape turns one of those into a group of
    paths with fresh ids, so the label's own id names the group and not the glyphs --
    and better than half the DIVE page labels are multi-line.
    """
    tree = ET.parse(flat_path)
    ids = []

    def walk(node, inherited):
        here = inherited or 'font-family' in (node.get('style') or '')
        if node.tag == f'{{{SVG}}}path' and here and node.get('id'):
            ids.append(node.get('id'))
        for kid in node:
            walk(kid, here)

    # Inherited, because a multi-line label becomes a <g> carrying the font style with
    # bare paths inside it.  Testing only the path would find none of them.
    walk(tree.getroot(), False)
    return ids


def embed_ids(f, name, ids):
    f.write('static const char *const %s[] = {\n' % name)
    for i in ids:
        f.write('    "%s",\n' % i)
    f.write('    nullptr,\n};\n\n')
...
def main():
    ...
    if args.text_to_path:
        ...
        if flat:
            text_ids['panel'] = text_shape_ids(flat)
    ...
    if pages and (args.text_to_path or args.background):
        ...
        for p in pages:
            if args.text_to_path:
                ...
                if flatten(p.path, out, drop_ids=p.rasters, offset=p.offset,
                    ...
                    text_ids[p.name] = text_shape_ids(out)
                    written.append(out)
            if args.background:
        ...
        with open(hdr, 'w') as f:
            ...
            # Which shapes on each page came from text.  Lettering is drawn in a
            # second pass with a finer antialiasing fringe; see kFringeSharpen.
            for p in pages:
                embed_ids(f, 'kDiveTextIds_%s' % p.name, text_ids.get(p.name, []))
            f.write('static const char *const *const kDiveTextIds[] = {\n')
            for p in pages:
                f.write('    kDiveTextIds_%s,\n' % p.name)
            f.write('};\n')
        written.append(hdr)

        if 'panel' in text_ids:
            hdr = os.path.join(args.out_dir, 'panel_text_ids.h')
            with open(hdr, 'w') as f:
                f.write('// GENERATED by plugin/tools/panel_export.py -- do not edit.\n'
                        '//\n'
                        '// Every shape in panel_flat.svg that used to be a <text>.  The\n'
                        '// UI draws these in a second pass with a finer antialiasing\n'
                        '// fringe than the curves get.\n'
                        '#pragma once\n\n')
                embed_ids(f, 'kPanelTextIds', text_ids['panel'])
            written.append(hdr)
