...
6. R8-SYSTEM-DESIGN.md: **Superseded by R8-CONVERSION.md, and left in place per the "mark obsolete rather than delete" rule.** Gemini's first look at the Roland R-8 / R-8M card architecture, from one card and unchecked against the bytes. Right about the architecture, wrong about most of the numbers.

7. R8-CONVERSION.md: The verified R-8 format, and the plan for converting R-8 sample data into U-110 cards. The headline is that R-8 and U-110 PCM data are the *same* format on the *same* wiring — proven by descrambling every dump with the permutations this project derived from U-110 firmware alone and matching the .opn/.ope images byte for byte — so sample bytes copy across with no transcoding at all. Also: each R-8 tone carries two sample blocks that the machine sums (a snare's noise in one, its shell tone in the other), which lands on the U-110's two partials almost exactly; the U-110 card layout constraints measured across all fifteen SN-U110 cards; and a scorecard for what the Gemini document got right and wrong. Fixed an error in ROM-ANALYSIS.md §6.6 on the way (correction #34).
